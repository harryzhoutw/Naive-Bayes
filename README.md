# Naive Bayes–based Unknown Data Classification

## 背景介紹
在開發刷卡系統時，硬體設備會即時將刷卡資料傳送至後端系統進行處理。然而，在實務運行中，偶爾會接收到硬體傳送格式異常或未定義的資料，且這些異常資料無法事先完整列舉，增加了系統判斷與處理的困難度。
...
// 4. 根據回傳結果進行後續處理
if (result.isValid) {
    std::cout << "RFID: " << rfid << " -> VALID (confidence: " << result.confidence << ")" << std::endl;
} else {
    std::cout << "RFID: " << rfid << " -> INVALID (" << result.reason << ")" << std::endl;
}
// 輸出範例: RFID: FFFFFFFF -> INVALID (log-likelihood=-336.27 < threshold=-6.63345)
```
